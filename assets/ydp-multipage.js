/* YDP Multi-page helper — ydp-multipage.js
 * Thin wrapper around /assets/govbb-framework.js for multi-file
 * prototypes that pass state between pages via sessionStorage. */
(function (global) {
  'use strict';
   ...
   * cfg: { formName, storeKey, pageId, title, template, nextUrl, validate }
  ...
  YDP.initPage = function (cfg) {
    ...
    // Override globals that GovBB helpers reference via onclick.
    window.back = function () { history.back(); };
    window.goBack = window.back;
    window.nav = function (target) {
      YDP.saveData();
      if (/\.html$/.test(target) || target.indexOf('/') !== -1) {
        window.location.href = target;
      }
    };
    window.goTo = window.nav;
    window.next = function () {
      var errors = cfg.validate ? (cfg.validate() || []) : [];
      if (errors.length) { GovBB.showErrors(errors); return; }
      GovBB.clearErrors();
      YDP.saveData();
      if (cfg.onBeforeNext) {
        var override = cfg.onBeforeNext();
        if (override) { window.location.href = override; return; }
      }
      if (cfg.nextUrl) window.location.href = cfg.nextUrl;
    };

    // Single-page flow so we get all the framework's input binding.
    var pages = {};
    pages[cfg.pageId] = cfg.template;
    GovBB.init({
  ...
  YDP.radioWithOther = function (name, label, options, opts) {
    opts = opts || {};
    var optsWithOther = options.slice();
    optsWithOther.push('Other');
    var body = GovBB.radioGroup(name, label, optsWithOther, { hint: opts.hint });
    var otherId = name + '-other';
    var shown = GovBB.D[name] === 'Other' ? '' : 'display:none;';
    body += '<div data-show-when-name="' + name + '" data-show-when-value="Other" style="' + shown + '" class="pl-s mt-s">' +
      GovBB.textField(otherId, opts.otherLabel || 'Please specify') +
    '</div>';
    return body;
  };

  /* Render a conditional reveal block shown when a named radio equals `value`. */
  YDP.revealWhen = function (name, value, innerHtml) {
    var shown = GovBB.D[name] === value ? '' : 'display:none;';
    return '<div data-show-when-name="' + name + '" data-show-when-value="' + value + '" style="' + shown + '" class="space-y-8 border-l-4 border-bb-blue-40 pl-s">' + innerHtml + '</div>';
  };

  /* Escape HTML for use inside templates. */
  YDP.esc = function (s) {
  ...
  /* Helper to format a value for a summary row. Arrays become comma-separated. */
  YDP.formatVal = function (v) {
  ...
  /* Collect a list of checkbox values given names like 'foo__Option'. */
  YDP.collectChecks = function (prefix) {
    var out = [];
    Object.keys(GovBB.D).forEach(function (k) {
      if (k.indexOf(prefix + '__') === 0 && GovBB.D[k]) {
        out.push(k.slice(prefix.length + 2));
      }
    });
    return out;
  };
  ...
  YDP.checkboxGroup = function (name, label, options, opts) {
    opts = opts || {};
    var hint = opts.hint ? '<p class="text-[1.25rem] leading-normal text-bb-mid-grey-00">' + YDP.esc(opts.hint) + '</p>' : '';
    var items = '';
    options.forEach(function (opt) {
      items += GovBB.checkboxItem(name + '__' + opt, opt);
    });
    var body = '<div class="field-group flex flex-col gap-s items-start w-full">' +
      '<p class="text-[1.25rem] leading-normal font-bold text-bb-black-00">' + YDP.esc(label) + '</p>' +
      hint + items + '</div>';
    if (opts.includeOther) {
      var otherId = name + '__Other';
      var otherTextId = name + '-other';
      body += GovBB.checkboxItem(otherId, 'Other');
      var shown = GovBB.D[otherId] ? '' : 'display:none;';
      body += '<div data-show-when-check="' + otherId + '" style="' + shown + '" class="pl-s mt-s w-full">' +
        GovBB.textField(otherTextId, 'Please specify') + '</div>';
    }
    return body;
  };
